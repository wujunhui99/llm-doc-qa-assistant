...
|---|---|---|
| Backend API contracts (`/backend/apps/api-go/*`, handlers, DTOs) | `ARCHITECTURE.md`, `docs/product-specs/*` | Update request/response behavior and endpoint impact. |
...
| Microservice topology/startup changes (`/backend/apps/*`, service ports, service split/merge) | `ARCHITECTURE.md`, `README.md`, `docs/RELIABILITY.md`, `docs/SECURITY.md` | Update service responsibilities, startup/runbook, dependency graph, and failure boundaries. |
...
- Bug-fix or refactor with **no external behavior change** can skip doc edits if all are true:
  - no API/RPC/schema/config/ops contract change,
  - no security/reliability policy change,
  - no product acceptance behavior change.
- When skipping docs for a bug-fix/refactor, record a short note in commit/PR description:
  - `docs: no update needed (internal fix only, no contract change)`.
